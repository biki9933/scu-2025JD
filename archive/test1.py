...
def main():
    ...
    spark = SparkSession.builder \
        .appName("FlightDW_ETL_For_Your_Project_v2") \
    ...
    final_segments_df = exploded_df.select(
        ...
        # 【修改】对所有用于关联的字符串键进行trim清理，增强代码健壮性
        F.trim(F.col("segment_departure_code")).alias("segment_departure_code"),
        F.trim(F.col("segments_arrival_airport_arr")[F.col("pos")]).alias("segment_arrival_code"),
        F.trim(F.col("segments_airline_name_arr")[F.col("pos")]).alias("airline_name"),
        F.trim(F.col("segments_airline_code_arr")[F.col("pos")]).alias("airline_code"),
    ...
    dim_airport_df = (airports_raw_df.select(
        ...
        ).filter(F.col("airport_code").isNotNull()).distinct()
    ...
    dim_airline_df = (final_segments_df.select("airline_code", "airline_name")
        .filter(F.col("airline_code").isNotNull()).distinct()
    ...
    dim_flight_details_df = (final_segments_df.select("is_refundable", "is_basic_economy", "is_non_stop")
        ...
        .select("flight_details_fk", "flight_number", "is_refundable", "is_basic_economy", "is_non_stop")
    )
