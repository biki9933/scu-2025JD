...
def main():
    """
    【数据清洗脚本】
    读取 'itinerary_details' 表，修正 full_path 列，并存入 'itinerary_details_cleaned' 新表。
    """
    # --- 1. 初始化 Spark Session ---
    ...
    try:
        ...
        # --- 5. 【核心修正】将修正后的数据写入新表 ---
        print(f"\n[步骤2] 开始将修正后的数据写入新表: {target_table_name}")
        
        # 直接使用 insertInto，去掉 partitionBy
        cleaned_df.write \
            .mode("overwrite") \
            .insertInto(target_table_name) # <<< 修正点：直接调用 insertInto
